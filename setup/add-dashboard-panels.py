...
def add_panels_to_dashboard(dashboard_id, panels):
    ...
    r = session.get(f"{KIBANA_URL}/api/saved_objects/dashboard/{dashboard_id}")
    if r.status_code != 200:
        print(f"  ✗ No se pudo obtener dashboard {dashboard_id}")
        return False
    
    dashboard = r.json()
    if "error" in dashboard:
        print(f"  ✗ Error: {dashboard['error']['message']}")
        return False
    
    # Actualizar con paneles
    dashboard['attributes']['panels'] = panels
    
    r = session.put(
        f"{KIBANA_URL}/api/saved_objects/dashboard/{dashboard_id}",
        json={"attributes": dashboard["attributes"]},
        timeout=10
    )
    
    if r.status_code not in [200, 201]:
        print(f"  ✗ Error {r.status_code} actualizando dashboard")
        return False
